...
def load_data_csv(inputfile, list_names, parse_dates_list):
    """
    Loads the data necessary for train/test from a csv file into a Pandas.DataFrame object
    :param inputfile: the input file
    :param list_names: the names of the columns
    :param parse_dates_list: the columns that should be parsed as Data
...
def read_dirs_csv(dir, cols, parse_dates_list):
    """
    Read the
    :param dir:
    :param cols:
...
if __name__ == "__main__":
    ...
    df_customers = read_dirs_csv(get_dir_customers(dir), COLS_CUSTOMERS, [4])
    df_receipts = read_dirs_csv(get_dir_receipts(dir), COLS_RECEIPTS, [5])
    df_returns = read_dirs_csv(get_dir_returns(dir), COLS_RETURNS, [5])
